...
def list_notes_from_midi(file_path):
    ...
    for track in midi.tracks:
        absolute_time = 0
        for msg in track:
            absolute_time += mido.tick2second(msg.time, midi.ticks_per_beat, mido.bpm2tempo(120))  # 187
            # absolute_time += dt
            ...
            if msg.type == 'note_on' and msg.velocity > 0:
                ...
                if all_notes == [] or not any(note.get("start_time") == absolute_time for note in all_notes if note.get("velocity") > 0):
                    all_notes.append({
                        'name': note_name,
